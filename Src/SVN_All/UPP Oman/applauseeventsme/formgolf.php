 ...
 * Last Modified: RR 21:32 Fri 02 March 2007
 * QVCS Version: $Revision: 1.116 $
 *
 * Read This First
 * ~~~~~~~~~~~~~~~
 ...
 *		line 3163 in this file - replace "yourhost\.com" with your mail server's
 ...
 *  1. you have more than one multi-page form on your website
 *  2. users can start one form multi-page form and then jump to another
 *     one (e.g. by clicking a link on your site's navigation)
 *  3. the user gets to the 2nd or later page of one multi-page form
 *     and then jumps to the start of the second multi-page form
 *  4. the values they had from the first multi-page form "infect" the
...
define("EMAIL_NAME","^[-a-z0-9.]+");	// the '^' is an important security feature!
    ...
    //
//$TARGET_EMAIL = array("^oerceogolf2010@infoline\.co.om$");

$TARGET_EMAIL = array("^jtracey@applauseeventsme\.com$",
	    "^sudha9490@gmail\.com$");
...
define("DEF_ALERT","formbackup@gmail.com");
...
$FROM_USER = "";                            // the default - setting not used
    ...
    //             the form.
    //
    // For Alert Messages (see above for Form Results):
    ...
    //
    // For Auto Responses (see above for Form Results):
    //      if the "FromAddr" feature is used in the "autorespond" field, then:
    //          set Sender and From Line values to the FromAddr value
    //      otherwise:
    //          if FROM_USER (in FormMail's configuration) is set and is not
    //          empty, then:
    //              if it has the value "NONE", then:
    ...
    //          PHP option.
    //      In every case:
    //          set the "From:" field of the email to the value of the From Line.
	//
...
define("CHECK_DAYS",30);
	...
	//
$SCRATCH_PAD = "/var/www/vhosts/applauseeventsme.com/httpdocs/tmp";
...
define("ALERT_ON_ATTACK_DETECTION",false);
    ...
    // crafted to try to fool FormMail into sending an email with a virus
...
define("ATTACK_DETECTION_SPECIALS",true);
	...
	//
	// ** OPTIONAL **
	// Set ATTACK_DETECTION_MANY_URLS to non-zero if you want FormMail's
    // attack detection to check for "many urls in a field" attack.
    //
    // This attack became prevalent in the latter part of 2006.  The spammer
    // parses your form looking for one or more textarea fields.  He then
    // crafts a fake submission and the textarea fields include a large number
    // of URLs (to their product pages).  In general, the submission will fail
    // but you might get an alert message containing the annoying failed
    // submission (depending on your other configuration settings).
    //
    // This detection allows you to control how many URLs are valid for data
    // entered into your fields.  FormMail cannot distinguish between
    // textarea and non-textarea fields (because of the way HTML forms work),
    // but it can look for a large number of URLs in any field.  You control
    // how many URLs will trigger the attack detection and cause the submission
    // to be ignored.
    //
    // The default of 0 disables this detection.  This is the best default
    // because you might want valid users to enter URLs into your forms!
    //
    // If your forms are designed to allow people to send you URLs, then you
    // need to decide how many URLs is too many and put this value in this
    // setting.
    //
    // For a general purpose contact form with a message area, a value of 2
    // or 3 is probably reasonable.
    //
    // Note also the similar setting below: ATTACK_DETECTION_MANY_URL_FIELDS
    // You can enable both of these attack detections together because they
...
define("ATTACK_DETECTION_MANY_URLS",0);
	...
	//
	// ** OPTIONAL **
	// Set ATTACK_DETECTION_MANY_URL_FIELDS to non-zero if you want
    // FormMail's attack detection to check for "many fields with
    // urls" attack.
    //
    // This attack is similar to the one detected with
    // ATTACK_DETECTION_MANY_URLS, except that instead of lots of URLs
    // in one field, there are lots of fields with URLs in them.
    //
    // This detection allows you to control how many fields on your forms
    // can contain URLs. You control how many fields such will trigger the
    // attack detection and cause the submission to be ignored.
    //
    // The default of 0 disables this detection.  This is the best default
    // because you might your forms to have several fields where it's
    // OK for users to enter URLs.
    //
    // If you never ask for URLs, set this value to 1.  For a general
    // purpose contact form with a message area, the user may want to send
    // you some URLs in the message area, so a value of 2 is appropriate
    // (meaning that if 2 or more fields have a URL, then spam is detected).
    //
    // Note also the similar setting above: ATTACK_DETECTION_MANY_URLS
    // You can enable both of these attack detections together because they
    // test for different things.
	//
define("ATTACK_DETECTION_MANY_URL_FIELDS",0);
	...
	//
	// ** OPTIONAL **
	// Set ATTACK_DETECTION_IGNORE_ERRORS to true if you want FormMail to
    // treat certain errors (e.g. no recipients field) as a spam attack.
    //
    // Spammers sometimes try to send you spam via FormMail by assuming
    // that FormMail will send you an alert if they don't fill some
    // fields correctly.
    //
    // For data fields, you can ignore these errors by setting
    // ALERT_ON_USER_ERROR to false.
    //
    // For other errors, however, set ATTACK_DETECTION_IGNORE_ERRORS
    // to true.
    //
    // NOTE: you don't want to do this UNLESS you've thoroughly tested
    // your forms.  The only way FormMail can tell you about faults in your
    // forms is with alerts.
    //
    // When you set ATTACK_DETECTION_IGNORE_ERRORS to true, you won't get
    // some important alerts any more, so errors will be verify hard to
    // figure out.
    //
    // NOTE: that this setting is different from the other attack detections
    // in that the user is *not* redirected to an abuse message page.
    // This setting purely prevents normal alerts being sent to you for
...
function SendAlert($s_error,$b_filter = true,$b_non_error = false)
{
    ...
    if (!empty($s_alert_to))
    {
        ...
        {
                //
                // some fields aren't security issues - show
                // those in the alert
                //
            $a_safe_data = array(
			...
			$s_new_mesg = implode(BODY_LF,$a_safe_data);
...
@	$log_fp = fopen($log_file,"a");
	...
	$entry = $date.":".$SPECIAL_VALUES["email"].",".
			$SPECIAL_VALUES[""].",".$SPECIAL_VALUES["subject"]."\n";
	fwrite($log_fp,$entry);
	fclose($log_fp);
}
...
function DetectManyURLsAttack($a_fields,&$s_attack,&$s_info)
{
    ...
    {
        if (IsSpecialField($s_fld))
                //
                // skip special fields because some have URLs in them
                //
            continue;
        if (isset($s_data) &&
            ...
            !empty($s_data))
        {
            $n_match = preg_match_all("/https{0,1}:\/\/|<\sa\shref=/msi",
                                        $s_data,$a_matches);
            if (!is_int($n_match))
                $n_match = 0;
