...
/*	Button Hover
/*------------------------------------------------*/
if (jQuery.browser.msie) { /* not supported bu older verisons of IE */
} else {
    jQuery(document).ready(function () {
...
/*------------------------------------------------*/
/*  Superfish Dropdown Menu (top-toolbar left)
/*------------------------------------------------*/
function tt_superFish() {
...
/* ------------------------------------------------------------------------
* jQuery Plugins included below
* Added these plugins to this file to reduce HTTP requests and speed up website loading
* Scripts below have been "minified" for better optimization
* To "un-minify" these scripts please check out: www.jsbeautifier.org
* Cheers :)
* ------------------------------------------------------------------------- */
...
; (function ($) {
    $.fn.superfish = function (op) {
        var sf = $.fn.superfish, c = sf.c, $arrow = $('<span class="' + c.arrowClass + '"> &#187;</span>'), over = function (e) { var $$ = $(this), menu = getMenu($$); clearTimeout(menu.sfTimer); $$.showSuperfishUl().siblings().hideSuperfishUl(); }, out = function (e) { var $$ = $(this), menu = getMenu($$), o = sf.op; var close = function () { o.retainPath = ($.inArray($$[0], o.$path) > -1); $$.hideSuperfishUl(); if (o.$path.length && $$.parents('li.' + o.hoverClass).length < 1) { o.onIdle.call(); $.proxy(over, o.$path, e)(); } }; if (e.type !== 'mouseleave' && e.type !== 'focusout') { close(); } else { clearTimeout(menu.sfTimer); menu.sfTimer = setTimeout(close, o.delay); } }, getMenu = function ($child) {
            ...
            var menu = $child.closest('.' + c.menuClass)[0]; sf.op = sf.o[menu.serial]; return menu;
        }, applyHandlers = function ($menu) {
            var targets = 'li:has(ul)'; if (!sf.op.useClick) { if ($.fn.hoverIntent && !sf.op.disableHI) { $menu.hoverIntent(over, out, targets); } else { $menu.on('mouseenter', targets, over).on('mouseleave', targets, out); } }
            $menu.on('focusin', targets, over).on('focusout', targets, out).on('click', 'a', clickHandler).on('touchstart', 'a', touchHandler);
        }, touchHandler = function () { var $$ = $(this); if (!$$.next('ul').is(':visible')) { $(this).data('follow', false); } }, clickHandler = function (e) { var $a = $(this), $submenu = $a.next('ul'), follow = ($a.data('follow') === false) ? false : true; if ($submenu.length && (sf.op.useClick || !follow)) { e.preventDefault(); if (!$submenu.is(':visible')) { $.proxy(over, $a.parent(), e)(); } else if (sf.op.useClick && follow) { $.proxy(out, $a.parent(), e)(); } } }, addArrows = function ($li, o) { if (o.autoArrows) { $li.children('a').each(function () { addArrow($(this)); }); } }, addArrow = function ($a) { $a.addClass(c.anchorClass).append($arrow.clone()); }; return this.addClass(c.menuClass).each(function () { var s = this.serial = sf.o.length; var o = $.extend({}, sf.defaults, op); var $$ = $(this); var $liHasUl = $$.find('li:has(ul)'); o.$path = $$.find('li.' + o.pathClass).slice(0, o.pathLevels).each(function () { $(this).addClass(o.hoverClass + ' ' + c.bcClass).filter('li:has(ul)').removeClass(o.pathClass); }); sf.o[s] = sf.op = o; addArrows($liHasUl, o); applyHandlers($$); $liHasUl.not('.' + c.bcClass).hideSuperfishUl(); o.onInit.call(this); });
    }; var sf = $.fn.superfish; sf.o = []; sf.op = {}; sf.c = { bcClass: 'sf-breadcrumb', menuClass: 'sf-js-enabled', anchorClass: 'sf-with-ul', arrowClass: 'sf-sub-indicator' }; sf.defaults = {
        hoverClass: 'sfHover',
